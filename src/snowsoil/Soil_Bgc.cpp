...
void Soil_Bgc::assignCarbonBd2Layer(){
 	...
 	while(currl!=NULL){
 		if(currl->isSoil){
 			...
 			currl->rawc  =	bd_sois->rawc[currl->solind-1];
 			currl->soma  =	bd_sois->soma[currl->solind-1];
 			currl->sompr =	bd_sois->sompr[currl->solind-1];
 			currl->somcr =	bd_sois->somcr[currl->solind-1];
 			currl->ch4   =	bd_sois->ch4[currl->solind-1];
 		}else{
			break;
 		}
 	...
 	ground->moss.dmossc = bd_sois->dmossc;
 	ground->organic.shlwc = bd_soid->shlwc;
 	ground->organic.deepc = bd_soid->deepc;
...
void Soil_Bgc::assignCarbonLayer2Bd(){
 	...
 	while(currl!=NULL){
 		if(currl->isSoil){
			...
			bd_sois->rawc[currl->solind-1] = currl->rawc;
			bd_sois->soma[currl->solind-1] = currl->soma;
			bd_sois->sompr[currl->solind-1]= currl->sompr;
			bd_sois->somcr[currl->solind-1]= currl->somcr;
			bd_sois->ch4[currl->solind-1]= currl->ch4;
...
void Soil_Bgc::prepareIntegration(const bool &mdnfeedback, const bool &mdavlnflg, const bool &mdbaseline){
	 ...
	 mossdeathn    = bd_v2soi->mossdeathn;

 	 // litter-fall C/N from Vegetation_bgc.cpp
 	 double blwlfc = bd_v2soi->ltrfalc[I_root];
 	 double abvlfc = fmax(0., bd_v2soi->ltrfalcall - blwlfc);
 	 double blwlfn = bd_v2soi->ltrfaln[I_root];
 	 double abvlfn = fmax(0., bd_v2soi->ltrfalnall - blwlfn);

     for(int i=0; i<cd_sois->numsl; i++){
    	 if (cd_sois->type[i]>0) {
    		 ltrflc[i] = abvlfc + bd_v2soi->rtlfalfrac[i] * blwlfc;   //always put the litter-falling in the first non-moss soil layer
    		 ltrfln[i] = abvlfn + bd_v2soi->rtlfalfrac[i] * blwlfn;

    		 abvlfc = 0.;
    		 abvlfn = 0.;
    	 } else {
    		 ltrflc[i] = bd_v2soi->rtlfalfrac[i] * blwlfc;   // root death is directly put into each soil layer
    		 ltrfln[i] = bd_v2soi->rtlfalfrac[i] * blwlfn;
    	 }

    	 if (ltrflc[i]>0. && ltrfln[i]> 0.) {
    		 bd_soid->ltrfcn[i] = ltrflc[i]/ltrfln[i];
    	 } else {
    		 bd_soid->ltrfcn[i] = 0.0;
    	 }
     ...
     if (nfeed) {
    	 ...
    	 totdzavln    = 0.;
    	 for(int i=0;i<cd_sois->numsl; i++){
    		 if((cd_sois->z[i]+cd_sois->dz[i]) <= ed_sois->draindepth){  //note: z is at the top of a layer
    			 totdzliq += fmax(0., ed_sois->liq[i]);
    			 totdzavln += fmax(0., bd_sois->avln[i]);
    		 } else {
    			 if (cd_sois->z[i]<ed_sois->draindepth){     // note: z is at the top of a layer
    				 double fdz = (ed_sois->draindepth - cd_sois->z[i])
         				    /cd_sois->dz[i];
    				 totdzliq += fmax(0., ed_sois->liq[i])*fdz;
    				 totdzavln += fmax(0., bd_sois->avln[i])*fdz;
    	 ...
    	 bd_a2soi->avlninput = 0.;
    	 if(avlnflag) {
//    		 bd_a2soi->avlninput += cd->ndep;
    	 }

    	 bd_a2soi->orgninput = 0.;
    	 if(cd->yrsdist<cd->gd->fri){
    		 bd_a2soi->orgninput += fd->fire_a2soi.orgn/12.;
    	 }
    	 if(!baseline){
//    		 bd_a2soi->orgninput += cd->orgnadd;
    	 }

     }

     // dead standing C due to fire will put into ground debris
     d2wdebrisc = bd_v2soi->d2wdebrisc;
     if (nfeed) d2wdebrisn = bd_v2soi->d2wdebrisn;

};

void Soil_Bgc::afterIntegration(){
	 for(int i=0;i<cd_sois->numsl; i++){
	 	bd_soid->tsomc[i] = bd_sois->rawc[i]+bd_sois->soma[i]
	 	                    +bd_sois->sompr[i]+bd_sois->somcr[i];
     }
...
void Soil_Bgc::initializeState(){
  ...
  //set initiate state variable
   ...
   bd_sois->wdebrisc = 0;
   bd_sois->dmossc   = dmossc;

   //initial N based on input total and SOM C profile
   double sumtotc = shlwc+deepc+minec;
   for (int il=0; il<MAX_SOI_LAY; il++ ){
	   double totc = bd_sois->rawc[il]+bd_sois->soma[il]
			           +bd_sois->sompr[il]+bd_sois->somcr[il];
	   if (totc > 0. && sumtotc > 0.) {
		   bd_sois->avln[il] = chtlu->initavln*totc/sumtotc;
		   bd_sois->orgn[il] = chtlu->initsoln*totc/sumtotc;
	   } else {
		   bd_sois->avln[il] = 0.;
		   bd_sois->orgn[il] = 0.;
	   }
	   bd_sois->ch4[il] = chtlu->initch4;
...
void Soil_Bgc::initializeState5restart(RestartData* resin){
	...
	for (int il =0; il<MAX_SOI_LAY; il++) {
		...
		bd_sois->rawc[il] = resin->rawc[il];
		bd_sois->soma[il] = resin->soma[il];
		bd_sois->sompr[il]= resin->sompr[il];
		bd_sois->somcr[il]= resin->somcr[il];

		bd_sois->orgn[il] = resin->orgn[il];
		bd_sois->avln[il] = resin->avln[il];

		bd_sois->ch4[il] = resin->ch4[il];
	...
	bd_sois->wdebrisc= resin->wdebrisc;
	bd_sois->wdebrisn= resin->wdebrisn;
	bd_sois->dmossc  = resin->dmossc;
...
void Soil_Bgc::deltac(){
    ...
    double ksomcr = 0.;    //for CR SOM (in model, somcr)
 	...
 	for (int il =0; il<cd_sois->numsl; il++){
		bd_soid->rhmoist[il] = getRhmoist(ed_soid->sws[il],  //Yuan: vwc normalized by total pore - this will allow respiration (methane/oxidation) implicitly
	 		   bgcpar.moistmin, bgcpar.moistmax, bgcpar.moistopt);
		bd_soid->rhq10[il] = getRhq10(ed_sois->ts[il]);
		...
		if(tmp_sois.rawc[il]>0.){
			del_soi2a.rhrawc[il] = (krawc * tmp_sois.rawc[il]
			                      * bd_soid->rhmoist[il] * bd_soid->rhq10[il]);
		} else {
	...
	kmoss = bgcpar.kdmoss;
   	...
   	if(tmp_sois.dmossc>0.){
	  	int il = 0;
   		double rhmoist = bd_soid->rhmoist[il];
		double rhq10   = bd_soid->rhq10[il];
   ...
   // for wood debris at ground surface
    del_soi2a.rhwdeb = 0.;
   	if(tmp_sois.wdebrisc>0){
		...
		double wdkd =0.;
	  	for (int il =0; il<cd_sois->numsl; il++){
	  		if(cd_sois->type[il]>0){//0 moss 1 shlw, and 2 deep
	  	  		rhmoist_wd =bd_soid->rhmoist[il] ;
	  	  		rhq10_wd = bd_soid->rhq10[il] ;
...
void Soil_Bgc::deltan(){
	...
	if (nfeed){ // soil-plant N cycle switched on
		...
		for(int i=0;i<cd_sois->numsl; i++){
	   		...
	   		double nimmob = getNimmob(ed_sois->liq[i], totc,
					                  tmp_sois.orgn[i], tmp_sois.avln[i],
						              bd_soid->knmoist[i], bgcpar.kn2);
	   		...
	   		if (cd_sois->type[i] == 0 && cd_sois->type[i+1] > 0){   // dead moss decomposition product is into the last moss layer
				if (tmp_sois.dmossc>0.)
		...
		totnextract = 0.;
		for (int il=0; il<MAX_SOI_LAY; il++) {
			totnextract += bd_soi2v->nextract[il];
		}
		...
		if (avlnflag){ // open-N (inorganic) swithed on - note here ONLY 'lost' considered, while 'input' shall be from outside if any
			...
			if(totdzliq>0){
				del_soi2l.avlnlost = totdzavln/totdzliq * bgcpar.fnloss *ed_soi2l->qdrain;
			}

			if(ed_sois->liq[0]>0){             // N loss with surface runoff water
				del_soi2l.avlnlost += tmp_sois.avln[0]/ed_sois->liq[0] * bgcpar.fnloss *ed_soi2l->qover;
			}

			if( del_soi2l.avlnlost > totdzavln - totnextract
       		                 + totnetnmin+ bd_a2soi->avlninput) {
			...
			del_soi2l.avlnlost = bd_a2soi->avlninput - totnextract
                                 +totnetnmin;
		}
		...
		if (baseline) {
			...
			del_soi2l.orgnlost = 0.;
	   		double tsomcsum=bd_soid->rawcsum+bd_soid->somasum
	   				+bd_soid->somprsum+bd_soid->somcrsum;
	   		double orgneven = tsomcsum/bgcpar.nmincnsoil;
	      	if ( orgneven >= bd_soid->orgnsum) {
	      		del_a2soi.orgninput += orgneven - bd_soid->orgnsum;
	      	} else {
	      		del_soi2l.orgnlost  += bd_soid->orgnsum - orgneven;
	      	}
...
void Soil_Bgc::deltastate(){
	...
	for(int il=0; il<cd_sois->numsl; il++){
		...
		if (cd_sois->type[il+1]>0 &&   //
				(il==0 || cd_sois->type[il]==0)){   // all products from dead moss C decomposition assumed into the last moss layer or first layer if no moss-layer
	...
	del_sois.dmossc = mossdeathc - del_soi2a.rhmossc*(1.0+somtoco2);
 	...
 	//fibric-C (rawc) will NOT to move between layers
   	...
   	double xtopdlthick  = fmin(0.10, cd_sois->deepthick);  //Yuan: the max. thickness of deep-C layers, which shallow-C can move into
	...
	for(int il =0; il<cd_sois->numsl; il++){
    	...
    	// 1) most of resistant-C increment into the fibric-horizon (generated above) will move down
	   	...
	   	if (cd_sois->type[il]<=1) {
 	   		...
 	   		if (il<(cd_sois->numsl-1) && cd_sois->type[il+1]>2) {
	   	...
	   	} else if (cd_sois->type[il]==2 && dlleft>0) {
   		// 2) s2dcarbon from above will move into the 'xtopdlthick';
	   		thickadded = fmin(cd_sois->dz[il], dlleft);
 	    ...
 	    //          (2) the mobilable portion is assumed to be related to decomposition activity,
   		...
   		} else if (cd_sois->type[il]==3) {
 	   		thickadded = fmin(cd_sois->dz[il], mlleft);
  	...
  	if(nfeed){
  	   	for(int il =0; il<cd_sois->numsl; il++){
  			...
  			if (cd_sois->type[il] == 0 && cd_sois->type[il+1] > 0){   // dead moss decomposition product is into the last moss layer
   			...
   			if (il==0){    // put the deposited orgn (here, mainly fire emitted or budget estimation) into the first soil layer
   				del_sois.orgn[il] += bd_a2soi->orgninput;
   			}

   			double dondrain = 0.;
   			if (totdzliq>0.01) {
   				if((cd_sois->z[il]+cd_sois->dz[il]) <= ed_sois->draindepth){  //note: z is at the top of a layer
  	   				dondrain = del_soi2l.orgnlost
	   		  		     *(ed_sois->liq[il]/totdzliq);
   				} else {
  	   				if (cd_sois->z[il]<ed_sois->draindepth){     // note: z is at the top of a layer
  	   					double fdz = (ed_sois->draindepth - cd_sois->z[il])
	          				         /cd_sois->dz[il];
  	   					dondrain = del_soi2l.orgnlost
	  	   		  		     *(ed_sois->liq[il]/totdzliq)*fdz;
  	   				}
   				}
   			}
   			del_sois.orgn[il] -= dondrain;
  	   		...
  	   		// inorganic N pools
   			double ninput = 0.;
   			if (il == 0) ninput = bd_a2soi->avlninput;
  	   		...
  	   		if (totdzliq>0.01) {
  	   			if((cd_sois->z[il]+cd_sois->dz[il]) <= ed_sois->draindepth){  //note: z is at the top of a layer
  	   				ndrain = del_soi2l.avlnlost
  	   		  		     *(ed_sois->liq[il]/totdzliq*ed_soi2l->qdrain);
  	   			} else {
  	   				if (cd_sois->z[il]<ed_sois->draindepth){     // note: z is at the top of a layer
  	   					double fdz = (ed_sois->draindepth - cd_sois->z[il])
  	           				         /cd_sois->dz[il];
  	  	   				ndrain = del_soi2l.avlnlost
  	  	   			  		     *(ed_sois->liq[il]/totdzliq*ed_soi2l->qdrain)*fdz;
...
void Soil_Bgc::deltaCH4Flux(const int &hours) {
	...
	for (il = 0; il < numsl; il++) {
		...
		if (ed_sois->watertab > 0.01) {
			torty_tmp = cd_sois->por[il] - ed_soid->lwc[il]
					  - ed_soid->iwc[il];
			if (torty_tmp < 0.05) torty_tmp = 0.05;
			torty = 0.66 * torty_tmp * pow(torty_tmp/cd_sois->por[il], 3.0);
			diff_tmp = diff_a;

		} else {
			torty = 0.66 * ed_soid->lwc[il] * pow(ed_soid->lwc[il]
					/ (cd_sois->por[il] + ed_soid->iwc[il]), 3.0);

			diff_tmp = diff_w;
		}

		diff[il] = diff_tmp * torty *
				   pow((ed_sois->ts[il] + 273.15)/293.15, 1.75);

		s[il] = cd_sois->dz[il] * cd_sois->dz[il] / (diff[il] * dt);
		r[il] = 2.0 + s[il];
	...
	for (j = 1; j <= m; j++) {
		...
		for (il = numsl - 1; il > 0; il--) {
			...
			TResp = getRhq10(ed_sois->ts[il]);
			if (ed_sois->ts[il] >= -2.0 && ed_sois->ts[il] < 0.1){
				TResp = getRhq10(0.0) * 0.3;
			} else if (ed_sois->ts[il] < -2.0) {
				TResp = 0.0;
			}
			for (int ip = 0; ip<=NUM_PFT; ip++) {
               if (cd_vegs->vegcov[ip]>0.) {
            	   rootfrac += cd_sois->frootfrac[il][ip];
            	   veglai += cd_vegs->lai[ip]*cd_vegs->vegcov[ip];
            	   tveg += cd_vegs->aerenchyma[ip]*cd_vegs->vegcov[ip];
               }
			}
			...
			if (ed_sois->watertab >
			     (cd_sois->z[il] + cd_sois->dz[il]/2.0)) {
				...
				//
				tmp = cd_sois->por[il] - ed_soid->lwc[il]-ed_soid->iwc[il];
				...
				Plant_m = Plant_m+Plant * tmp * cd_sois->dz[il] * 1000.0;           // umol/m2/hour (layerly summed)
				...
				Prod = Prod / 12.0
						/(cd_sois->dz[il] * cd_sois->por[il]);            // umol/L/hour

				//
				Oxid = 0.0;

				//
				Pressure = DENLIQ * G * (cd_sois->z[il] + cd_sois->dz[il]
						/ 2.0) + Pstd;
				SB = 0.05708 - 0.001545 * ed_sois->ts[il] + 0.00002069
						* ed_sois->ts[il] * ed_sois->ts[il];               //volume
				SM = Pressure * SB / (GASR * (ed_sois->ts[il]+273.15));      //mass
				Ebul = (tmp_sois.ch4[il] - SM) * bgcpar.rebul;                          // umol/L/hour
				if (Ebul < 0.0)	Ebul = 0.0;
				Ebul_m = Ebul_m+
						Ebul * ed_soid->lwc[il] * cd_sois->dz[il]*1000.0; // umol/m2/hour (layerly summed)

				//
				if (ed_sois->ts[il] > 0.0) {
					Plant_m = Plant_m+Plant * cd_sois->por[il] * cd_sois->dz[il]*1000.0;
				} else {
					Plant_m = Plant_m+Plant * ed_soid->lwc[il] * cd_sois->dz[il]*1000.0;
				}
			}
			...
			del_soi2a.Prod_m[il]  = del_soi2a.Prod_m[il]+
					           Prod*cd_sois->por[il]*cd_sois->dz[il]*1000.0*12.0e-6;    // kgC/m2/time-step
			del_soi2a.Oxid_m[il]  = del_soi2a.Oxid_m[il]+
					           Oxid*cd_sois->por[il]*cd_sois->dz[il]*1000.0*12.0e-6;    // kgC/m2/time-step
		...
		tmp_flux = diff[1] * (tmp_sois.ch4[1] - ub) / cd_sois->dz[1];
		if (tmp_flux < 0.0) tmp_flux = 0.0;                        // umol/L/hour
		Flux2A = Flux2A + tmp_flux;                                // umol/L/time-step
		tmp = (cd_sois->por[1] - ed_soid->lwc[1] - ed_soid->iwc[1]);
		if (tmp < 0.05) tmp = 0.05;
		Flux2A_m = Flux2A * tmp * cd_sois->dz[1] * 1000.0;       // umol/m2/time-step
...
void Soil_Bgc::setBgcData(BgcData* bdp){
 	...
 	if(tstepmode == MONTHLY){
 		bd_sois = &bdp->m_sois;
 		bd_soid = &bdp->m_soid;
 		bd_a2soi= &bdp->m_a2soi;
 		bd_v2soi= &bdp->m_v2soi;
 		bd_soi2a= &bdp->m_soi2a;
 		bd_soi2v= &bdp->m_soi2v;
 	}else if(tstepmode == DAILY){
 		bd_sois = &bdp->d_sois;
 		bd_soid = &bdp->d_soid;
 		bd_a2soi= &bdp->d_a2soi;
 		bd_v2soi= &bdp->d_v2soi;
 		bd_soi2a= &bdp->d_soi2a;
